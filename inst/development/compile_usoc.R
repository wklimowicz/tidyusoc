...
usoc_convert(
  usoc_directory = "../data_usoc_raw/spss25",
  new_directory = "../data_usoc_rds/",
  incremental = FALSE,
...
indresp[is.na(age), .N, waveid]
indresp[is.na(sex), .N, waveid]
indresp[is.na(race), .N, waveid]
...
usoc[, sex := fcase(
  ...
  sex == "male", "Male",
  sex == "proxy", "proxy",
  rep(TRUE, nrow(usoc)), as.character(usoc$sex)
) # Hack to recycle fcase
|> as.factor()]
